...
def main():
    ...
    indeces1 = re.compile("(\d{4})")    
    indeces2 = re.compile("(\d{3})")    
    ...
    L_plates = [''.join(re.findall(plates1, x)) or 
                ...
                ''.join(re.findall(plates7, x))  
                for x in L_vehicles]
    L_indeces = [''.join(re.findall(indeces1, x)) or 
                ''.join(re.findall(indeces2, x)) 
                for x in L_vehicles]

    L_indeces = [x.replace('186', '') for x in L_indeces]
    
    data = pd.DataFrame(zip(L_vehicles, L_plates, L_indeces))
